...
#print(X)
y = df_encoded['Crack spacing (mm)'] #output variable
#y = df.iloc[:, 8].values 
#print(y)
#Check for missing values
#print(df.isnull().sum())
...
plt.yticks([0.940, 0.942, 0.944, 0.946, 0.948, 0.950])
plt.xticks([0, 10, 20, 30, 40, 50])
#plt.grid(True)
plt.tight_layout()
plt.savefig('RF covert curves.JPG', dpi=500, bbox_inches = 'tight')
plt.show()
df = pd.DataFrame({
...
# Export to Excel
df.to_excel("RF_cumulative_best_plot.xlsx", index=False)
