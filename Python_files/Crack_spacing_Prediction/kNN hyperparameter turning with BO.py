...
rf = KNeighborsRegressor(metric='minkowski')
...
#plt.grid(True)
plt.tight_layout()
plt.savefig('KNN covert curves.JPG', dpi=500, bbox_inches = 'tight')
plt.show()
df = pd.DataFrame({
...
# Export to Excel
df.to_excel("KNN_cumulative_best_plot.xlsx", index=False)
