...
shap.initjs()
# Define dataset
Dataset="Database for crack spacing with no fiber type 241.csv"
...
print(df.isnull().sum())
from sklearn.ensemble import RandomForestRegressor
model = RandomForestRegressor(random_state = 42, max_depth=100,max_features=1, max_leaf_nodes=131, n_estimators=5)
model.fit(X, y)
...
disp1 = PartialDependenceDisplay.from_estimator(model, X, [6], kind="both", #BOTH
                                                ...
                                                pd_line_kw={'color':'blue','linewidth':3,'linestyle':'-'},
...
plt.ylabel(" Crack spacing (mm)",font='Times New Roman',size=18)
#plt.xlim(50,250)
