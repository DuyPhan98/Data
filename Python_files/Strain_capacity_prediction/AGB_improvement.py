...
regressor = AdaBoostRegressor(learning_rate=0.0106,n_estimators=6)
#Result of K fold
...
# Export to Excel
results_df.to_excel('cross_validation_results from AGB model.xlsx', index=False, engine='openpyxl')

print("Cross-validation results saved to 'cross_validation_results from AGB model.xlsx'")
...
pd.DataFrame(y_train, columns=['Experiment training_AGBBO']).to_csv('Experiment training_AGBBO.csv')
pd.DataFrame(y_pred, columns=['Prediction training_AGBBO']).to_csv('Prediction training_AGBBO.csv')
# Collection  testing data
y_test= np.array(y_test)
print(y_predtest)
pd.DataFrame(y_test, columns=['Experiment testing_AGBBO']).to_csv('Experiment testing_AGBBO.csv')
pd.DataFrame(y_predtest, columns=['Prediction testing_AGBBO']).to_csv('Prediction testing_AGBBO.csv')
...
plt.legend(["Experiment","Pure AGB"])
plt.xlim(0, 350)
plt.ylim(0,10)
