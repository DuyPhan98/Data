...
# Export to Excel
results_df.to_excel('cross_validation_results from KNN model.xlsx', index=False, engine='openpyxl')

print("Cross-validation results saved to 'cross_validation_results from KNN model.xlsx'")
...
pd.DataFrame(y_train, columns=['Experiment training_KNNBO']).to_csv('Experiment training_KNNBO.csv')
pd.DataFrame(y_pred, columns=['Prediction training_KNNBO']).to_csv('Prediction training_KNNBO.csv')
# Collection  testing data
y_test= np.array(y_test)
print(y_predtest)
pd.DataFrame(y_test, columns=['Experiment testing_KNNBO']).to_csv('Experiment testing_KNNBO.csv')
pd.DataFrame(y_predtest, columns=['Prediction testing_KNNBO']).to_csv('Prediction testing_KNNBO.csv')
...
plt.legend(["Experiment","Pure KNNBO"])
plt.xlim(0, 350)
plt.ylim(0,10)
